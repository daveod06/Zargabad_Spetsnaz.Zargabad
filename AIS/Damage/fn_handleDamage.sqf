...
_new_damage = if (_hitPartIndex >= 0) then {_damage - (_unit getHitIndex _hitPartIndex)} else {_damage - (damage _unit)};
...
if (_damageType isEqualTo "crash") then {
	...
	_min = [1, 0.89] select (_hitSelection isEqualTo "head");
...
{
	if (isNull _new_damage) then
	{
		_new_damage = 0.0;	
	};
};

diag_log format ["_new_damage: %1 --- AIS_DAMAGE_TOLLERANCE_FACTOR: %2 --- _hitPart select 2: %3 ---", _new_damage, AIS_DAMAGE_TOLLERANCE_FACTOR, _hitPart select 2];
...
diag_log format ["_hitSelection: %1 --- _damage: %2 --- _hitPart: %3 --- _hitPartIndex: %4 --- _projectile: %5 ----> %6", _hitSelection, _damage, _hitPart , _hitPartIndex, _projectile, diag_frameno];
_damage = _damage min 0.89;
